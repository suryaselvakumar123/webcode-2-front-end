            ...
            <div className='row'>
                <div className="col-md-12" style={{ justifyItems: 'center' }}>
                    {movie.map((object, index) => (
                        <container key={index}>
                            <row>
                                <Card style={{ width: '20rem', display: 'inline-flex', marginLeft: '50px', marginTop: '50px' }}>
                                    ...
                                    <Card.Body>
                                        ...
                                        <Tooltip title="Edit Movie" arrow>
                                            <Button variant="success" onClick={() => navigate(`/edit/${object.id}`)}><EditIcon /></Button>
                                        </Tooltip>&nbsp;&nbsp;

                                        <Tooltip title="Delete Movie" arrow>
                                            <Button variant="danger" onClick={() => {
                                                const filteredMovies = movie.filter((item) => item.id !== object.id);
